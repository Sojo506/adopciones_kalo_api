    ...
    */
    ...
    BEGIN
        OPEN V_CURSOR_RESULTADO FOR
            ...
            JOIN FIDE_ESTADO_TB E ON C.ID_ESTADO = E.ID_ESTADO
            WHERE C.ID_ESTADO = 1
              AND (C.FECHA_INICIO IS NULL OR TRUNC(SYSDATE) >= TRUNC(C.FECHA_INICIO))
              AND (C.FECHA_FIN IS NULL OR TRUNC(SYSDATE) < TRUNC(C.FECHA_FIN))
    ...
    BEGIN
        OPEN V_CURSOR_RESULTADO FOR
            ...
            LEFT JOIN (
                ...
                WHERE EV.ID_ESTADO != 2
                GROUP BY EV.ID_SEGUIMIENTO
            ) EV ON SG.ID_SEGUIMIENTO = EV.ID_SEGUIMIENTO
            WHERE C.ID_CUENTA = P_ID_CUENTA
              AND SG.ID_ESTADO = 1
              AND (SG.FECHA_FIN IS NULL OR TRUNC(SG.FECHA_FIN) >= TRUNC(SYSDATE))
    ...
    /* PROCEDURE FIDE_CAMPANIA_TB DESACTIVAR VENCIDAS */

    PROCEDURE FIDE_DESACTIVAR_CAMPANIAS_VENCIDAS_SP
    IS
        V_ESTADO_INACTIVO FIDE_ESTADO_TB.ID_ESTADO%TYPE;
    BEGIN
        SELECT MIN(ID_ESTADO)
        INTO V_ESTADO_INACTIVO
        FROM FIDE_ESTADO_TB
        WHERE UPPER(NOMBRE_ESTADO) = 'INACTIVO';

        IF V_ESTADO_INACTIVO IS NULL THEN
            RAISE_APPLICATION_ERROR(-20004, 'No existe el estado Inactivo.');
        END IF;
        ...
        WHERE ID_ESTADO = 1
          ...
          AND TRUNC(FECHA_FIN) <= TRUNC(SYSDATE);
    ...
    PROCEDURE FIDE_DESACTIVAR_SEGUIMIENTOS_VENCIDOS_SP
    IS
        V_ESTADO_INACTIVO FIDE_ESTADO_TB.ID_ESTADO%TYPE;
    BEGIN
